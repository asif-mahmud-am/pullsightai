...
def evaluate_review(diff: str, agent_review: str) -> dict:
    judge_prompt = f"""
    Diff:
    {diff}
    ...
    Evaluate this review on:
    - Correctness (did it catch real issues?)
    - Coverage (does it address enough of the diff?)
    - Actionability (are the suggestions useful?)
    - Conciseness (is it clear and not verbose?)

    Give each score 1–5 and return JSON like:
    {{
      "correctness": 4,
      "coverage": 3,
      "actionability": 5,
...
def log_evaluation(diff, agent_review: str, pr_id):
    diff = diff

    # Step 2: Evaluate review
    evaluation = evaluate_review(diff, agent_review)
    scores = json.loads(evaluation)

    # Step 3: Log in MLflow
    with mlflow.start_run():
        mlflow.log_param("model", "meta-llama/llama-4-scout-17b-16e-instruct")
        mlflow.log_param("pr_id", pr_id)

        for metric, score in scores.items():
            mlflow.log_metric(metric, score)

        # Save raw outputs
        with open("agent_review.txt", "w") as f:
            f.write(agent_review)
        mlflow.log_artifact("agent_review.txt")

        with open("evaluation.json", "w") as f:
            json.dump(scores, f)
        mlflow.log_artifact("evaluation.json")
...
async def process_pr_review_background(extracted_data: dict):
    ...
    if extracted_data["prFiles"]:
        ...
        evaluate_review(chunks, summary.pr_summary, extracted_data["pullRequestAnalysisId"])
